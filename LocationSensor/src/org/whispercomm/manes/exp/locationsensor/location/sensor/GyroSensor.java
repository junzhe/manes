...
public class GyroSensor extends MotionPositionSensor<Gyro> {

	public static final String TAG = GyroSensor.class.getSimpleName();
	public static final long MEASURE_PERIOD = 20000;
	private static final long MEASURE_DURATION = 150;
	private static final int SENSOR_TYPE = Sensor.TYPE_GYROSCOPE;
	private static final SensorSignal OPERATOR_SIGNAL = SensorSignal.GYRO_AVAILABLE;

	public GyroSensor(Context context, Handler handler,
			SensorManager sensorManager, GyroFactory readingFactory) {
		super(context, handler, sensorManager, readingFactory,
				MEASURE_DURATION, SENSOR_TYPE, OPERATOR_SIGNAL);
	}

	@Override
	synchronized public Gyro getLatestReading() {
		// TODO is it a right thing to deplete the record here?
		Gyro reportReadings = new Gyro();
...
//			Log.i(TAG, "new gyro readings: " + data.toString());
		}
	}
}
