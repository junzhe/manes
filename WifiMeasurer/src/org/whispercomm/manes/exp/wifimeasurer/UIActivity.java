...
public class UIActivity extends Activity {
	...
	public static final String TAG = "org.whispercomm.manes.exp.wifimeasurer.UIActivity";
	/**
	 * The height of the debug terminal in line count.
	 */
	public static final int TEXTVIEW_HEIGHT_IN_LINE_COUNT = 5;
	...
	private void initializeGUI() {
		...
		hint.append("Please enter wifi scan period in seconds:");
		ll.addView(hint);
		
		editText = new EditText(this);
		ll.addView(editText);
		
		hint2 = new TextView(this);
		hint2.append("Please enter wifi operation mode when waking up (1,2):");
		ll.addView(hint2);
	...
	private void initializeStart() {
		start.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				String wifiModeString = editText2.getText().toString();
				int wifiMode = 0;
				try {
					wifiMode = Integer.parseInt(wifiModeString);
				} catch (NumberFormatException e) {
					Log.e(TAG, e.getMessage(), e);
				}
				if (scanPeriod > 0 && wifiMode > 0) {
					// create sensor
					sensor = new WifiSensor(UIActivity.this, uiHandler, wifiMode);
					// start measurement
					...
					Toast toast = Toast.makeText(getApplicationContext(),
							"Start wifi scanning every " + scanPeriodString
									+ " seconds!", Toast.LENGTH_LONG);
					toast.show();
				} else {
					// make a warning toast
					Toast toast = Toast.makeText(getApplicationContext(),
							"Please input a valid scan period and wifi mode",
							Toast.LENGTH_LONG);
					toast.show();
				}
			}
