...
public class UdpServer {
	...
	private final static Logger LOGGER = Logger.getLogger(UdpServer.class
			.getName());
	...
	private DatagramPacket packet = new DatagramPacket(new byte[BUF_SIZE],
			BUF_SIZE); // Shared datagram
	...
	public synchronized void reset() {
		...
		case STARTED:
			this.addPropertyChangeListener(STATE_PROP,
					new PropertyChangeListener() {
						public void propertyChange(PropertyChangeEvent evt) {
							State newState = (State) evt.getNewValue();
							if (newState == State.STOPPED) {
								UdpServer server = (UdpServer) evt.getSource();
								server.removePropertyChangeListener(STATE_PROP,
										this);
								server.start();
							} // end if: stopped
						} // end prop change
		...
		try {
			...
			if (gg != null) {
				...
				for (String p : proposed) { // See which ones are valid
					try {
						...
						LOGGER.warning("Could not join " + p
								+ " as a multicast group: " + exc.getMessage());
					} // end catch
			...
			while (!this.mSocket.isClosed()) {
				...
				if (!this.mSocket.isClosed()) {
					...
					// ////// B L O C K I N G
					this.packet.setLength(BUF_SIZE);
			...
			synchronized (This) {
				if (this.currentState == State.STOPPING) { // User asked to stop
					...
					LOGGER.log(Level.WARNING, "Server closed unexpectedly: "
							+ exc.getMessage(), exc);
				} // end else
			} // end sync
	...
	public synchronized void send(DatagramPacket packet) throws IOException {
		if (this.mSocket == null) {
			throw new IOException(
					"No socket available to send packet; is the server running?");
		} else {
	...
	public synchronized int getReceiveBufferSize() throws SocketException {
		if (this.mSocket == null) {
			throw new SocketException(
					"getReceiveBufferSize() cannot be called when the server is not started.");
		} else {
	...
	public synchronized void setReceiveBufferSize(int size)
		...
		if (this.mSocket == null) {
			throw new SocketException(
					"setReceiveBufferSize(..) cannot be called when the server is not started.");
		} else {
	...
	public synchronized void setPort(int port) {
		if (port < 0 || port > 65535) {
			throw new IllegalArgumentException(
					"Cannot set port outside range 0..65535: " + port);
	...
	protected synchronized void fireUdpServerPacketReceived() {
		...
		UdpServer.Listener[] ll = listeners
				.toArray(new UdpServer.Listener[listeners.size()]);
		for (UdpServer.Listener l : ll) {
			try {
				l.packetReceived(event);
			} catch (Exception exc) {
				LOGGER.log(Level.WARNING, "UdpServer.Listener " + l
	 ...
	 *            new value
	 */
	protected synchronized void firePropertyChange(final String prop,
			final Object oldVal, final Object newVal) {
		try {
			propSupport.firePropertyChange(prop, oldVal, newVal);
		} catch (Exception exc) {
			LOGGER.log(Level.WARNING,
					"A property change listener threw an exception.", exc);
			fireExceptionNotification(exc);
		} // end catch
	} // end fire
