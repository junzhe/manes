...
public class GpsSensor implements LocationSensor<GPS>, LocationListener {
	...
	public void startSensing() {
		...
		try {
			if (isWorking == false && gpsActuator == null) {
				...
				gpsActuator.startOneMeasureAt(System.currentTimeMillis());
			}
		} finally {
	 ...
	 */
	@Override
	public void updateReadings(GPS location) {
	...
	public void onLocationChanged(Location location) {
		...
		updateReadings(new GPS(location.getLatitude(), location.getLongitude()));
	}
	...
	public void startOneMeasureBy(long execTime) {
		...
		try {
			if (isWorking == true)
				gpsActuator.startOneMeasureBy(execTime);
		} finally {
			isWorkingLock.unlock();
		}
	}
	...
	public void startOneMeasureAt(long execTime) {
		...
		try {
			if (isWorking == true)
				gpsActuator.startOneMeasureAt(execTime);
		} finally {
			isWorkingLock.unlock();
		}
	}
