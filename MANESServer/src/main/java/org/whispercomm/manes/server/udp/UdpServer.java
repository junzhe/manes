...
public class UdpServer {
	...
	private DatagramPacket packet = new DatagramPacket(new byte[64 * 1024],
			64 * 1024); // Shared datagram
	...
	protected synchronized void fireUdpServerPacketReceived() {
		...
		for (UdpServer.Listener l : ll) {
			try {
				...
				LOGGER.warning("UdpServer.Listener " + l
						+ " threw an exception: " + exc.getMessage());
				fireExceptionNotification(exc);
			} // end catch
	...
	protected synchronized void firePropertyChange(final String prop,
		...
		try {
			...
			LOGGER.log(
					Level.WARNING,
					"A property change listener threw an exception: "
							+ exc.getMessage(), exc);
			fireExceptionNotification(exc);
		} // end catch
	} // end fire
